...
const CustomizeReq = () => {
    ...
    const customization = (event) => {
        ...
        if (loggeduser) {
            ...
            addDoc(collection(db, `customizereq` ), {
                producttitle: product.producttitle,
                producttype: product.producttype,
                price: product.price,
                height, width, length, color, material, fabric, description
            }) 
            .then(() => {
      ...
      if (loggeduser) {
          ...
          addDoc(collection(db, `cart-${loggeduser[0].uid}` ), {
              ...
              newheight:height, newwidth:width, newlength:length, newcolor:color,
               newmaterial:material, newfabric:fabric, description,
              producttext:"Customize product",quantity:1
          }) 
          .then(() => {
